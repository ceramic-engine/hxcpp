...
#include <sys/stat.h>
...
#else
   #include <errno.h>
   #if !defined(EPPC) && !defined(HXCPP_NO_UNISTD)
      #include <unistd.h>
      #include <dirent.h>
      #include <termios.h>
      #include <sys/time.h>
   ...
   #ifndef ANDROID
      #include <locale.h>
      #if !defined(BLACKBERRY) && !defined(EPPC) && !defined(GCW0) && !defined(__GLIBC__) && !defined(HXCPP_NO_UNISTD)
         #include <xlocale.h>
      #endif
   #endif
#endif
...
{
   #if defined(HX_WINRT) || defined(HXCPP_NO_ENVIRON)
      return String();
   #else
...
{
#if defined(ANDROID) || defined(GCW0) || defined(HXCPP_NO_LOCALE)
    return false;
#else
...
String _hx_std_get_cwd()
{
   ...
   #elif defined(EPPC) || defined(HXCPP_NO_FILESYSTEM)
   return String();
   #else
#ifdef NEKO_WINDOWS
   wchar_t buf[261];
...
{
   #if !defined(HX_WINRT) && !defined(EPPC) && !defined(HXCPP_NO_FILESYSTEM)
#ifdef NEKO_WINDOWS
...
{
#if defined(HXCPP_PLATFORM_NAME)
   return HX_CSTRING(HXCPP_PLATFORM_NAME);
...
{
   #if defined(HX_WINRT) || defined(__EMSCRIPTEN__) || defined(EPPC) || defined(IPHONE) || defined(APPLETV) || defined(HX_APPLEWATCH) || defined(HXCPP_NO_SYSTEM)
   return -1;
   #else
...
{
   #if !defined(EPPC) && !defined(HXCPP_NO_FILESYSTEM)
   hx::EnterGCFreeZone();
...
#elif defined(EPPC) || defined(HXCPP_TIME_USE_CLOCK)
...
{
#if defined(HX_WINRT) || defined(__EMSCRIPTEN__) || defined(EPPC) || defined(HXCPP_NO_TERMINAL)
   return 0;
#elif defined(NEKO_WINDOWS)
   hx::EnterGCFreeZone();
