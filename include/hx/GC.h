...
inline int ConcurrentGcClampArrayScan(const void *inBase, int inLength, int inElemSize)
{
   extern HXCPP_EXTERN_CLASS_ATTRIBUTES int gConcurrentGcMarkActive;
   if (gConcurrentGcMarkActive && inLength>0)
   {
...
#define HX_GC_CONCURRENT_CLAMP_ARRAY(base,len,elemSize)    len = ::hx::ConcurrentGcClampArrayScan(base,len,elemSize);    if (len<=0) return;
#else
#define HX_GC_CONCURRENT_CLAMP_ARRAY(base,len,elemSize)
#endif
...
// HXCPP_GC_CONCURRENT - mostly-concurrent major collections on top of the
//  generational collector.  While a concurrent mark cycle is running
//  (gConcurrentGcMarkActive), the compiler-inserted write barriers switch from
...
#error "HXCPP_GC_CONCURRENT requires HXCPP_GC_GENERATIONAL (add -D HXCPP_GC_GENERATIONAL)"
#endif
#ifdef HXCPP_SINGLE_THREADED_APP
#error "HXCPP_GC_CONCURRENT is not supported with HXCPP_SINGLE_THREADED_APP"
#endif
#ifdef HXCPP_GC_VERIFY
#error "HXCPP_GC_VERIFY is not supported with HXCPP_GC_CONCURRENT"
#endif
#ifdef HXCPP_GC_DEBUG_ALWAYS_MOVE
#error "HXCPP_GC_DEBUG_ALWAYS_MOVE is not supported with HXCPP_GC_CONCURRENT"
#endif

// Marks-allocs use the full 6 bits
#define HX_GC_CONCURRENT_MARK_BIT_MASK 0x3f
...
HXCPP_EXTERN_CLASS_ATTRIBUTES extern int gConcurrentGcMarkActive;
...
HXCPP_EXTERN_CLASS_ATTRIBUTES void ConcurrentGcShade(void *inValue, hx::StackContext *inCtx);
...
HXCPP_EXTERN_CLASS_ATTRIBUTES void ConcurrentGcDirtyObject(hx::Object *inObj, hx::StackContext *inCtx);

// Write an "allocate black" header (full mark id, alloc-start + shadow row
//  marks) for an object freshly bump-allocated at inBuffer.
HXCPP_EXTERN_CLASS_ATTRIBUTES void ConcurrentGcAllocBlackHeader(hx::ImmixAllocator *inAlloc, unsigned char *inBuffer, int inSize, bool inContainer);
   ...
   inline static void *alloc(ImmixAllocator *alloc, size_t inSize, bool inContainer, const char *inName )
   {
         ...
         else
         {
            ...
            if (::hx::gConcurrentGcMarkActive)
               ::hx::ConcurrentGcAllocBlackHeader(alloc, buffer, (int)inSize, inContainer);
            else
            #endif
            if (inContainer)
...
#define HX_GC_CONCURRENT_SHADE_CTX(value,ctx) { \
   if (value) { \
      unsigned char _hx_fshade_mark = ((unsigned char *)(value))[ HX_ENDIAN_MARK_ID_BYTE ]; \
      if ( !(_hx_fshade_mark & HX_GC_CONST_ALLOC_MARK_BIT) && \
            (_hx_fshade_mark & HX_GC_CONCURRENT_MARK_BIT_MASK) != (::hx::gByteMarkID & HX_GC_CONCURRENT_MARK_BIT_MASK) ) \
         ::hx::ConcurrentGcShade((void *)(value),(::hx::StackContext *)(ctx)); \
   } }

#define HX_GC_CONCURRENT_DIRTY_CTX(obj,ctx) { \
   ...
   if ( !(_hx_fdirty_mark & (HX_GC_REMEMBERED|HX_GC_CONST_ALLOC_MARK_BIT)) ) \
      ::hx::ConcurrentGcDirtyObject((hx::Object *)(obj),(::hx::StackContext *)(ctx)); \
   }
...
#define HX_OBJ_WB_CONCURRENT_BULK(obj) { \
   if (::hx::gConcurrentGcMarkActive) HX_GC_CONCURRENT_DIRTY_CTX(obj,HX_CTX_GET); }

#else  // !HXCPP_GC_CONCURRENT
...
#define HX_OBJ_WB_CONCURRENT_BULK(obj)
...
#if defined(HXCPP_GC_GENERATIONAL) && defined(HXCPP_GC_CONCURRENT)
  ...
  #define HX_OBJ_WB_CTX(obj,value,ctx) { \
     if (::hx::gConcurrentGcMarkActive) { \
        HX_GC_CONCURRENT_SHADE_CTX(value,ctx) \
     } else { \
  ...
  #define HX_OBJ_WB_PESSIMISTIC_CTX(obj,ctx) { \
     if (::hx::gConcurrentGcMarkActive) { \
        HX_GC_CONCURRENT_DIRTY_CTX(obj,ctx) \
     } else { \
        unsigned char &mark =  ((unsigned char *)(obj))[ HX_ENDIAN_MARK_ID_BYTE]; \
  ...
  #define HX_OBJ_WB_NEW_MARKED_OBJECT(obj) { \
     if (::hx::gConcurrentGcMarkActive) { \
        HX_GC_CONCURRENT_DIRTY_CTX(obj,HX_CTX_GET) \
     } else \
